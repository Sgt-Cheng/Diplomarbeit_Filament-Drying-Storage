...
class WritePromptScreen(Frame):
    def __init__(self, parent, controller: 'App'):
        super().__init__(parent)
        self.controller = controller
        Label(self, text="Write Text to Tag", font=("Arial", 14, "bold")).pack(pady=(16, 4))
        self.lbl_uid = Label(self, text="UID: ?")
        self.lbl_uid.pack()
        Label(self, text=f"Max {USER_BYTES} bytes (UTF-8), will be padded to 4-byte words.").pack(pady=(0, 8))
        self.text = Text(self, height=10, wrap="word")
        self.text.pack(fill=BOTH, expand=True, padx=12, pady=8)
        btns = Frame(self)
    ...
    def _do_write(self):
        if not self._uid:
            ...
            return
        txt = self.text.get(1.0, END).rstrip("\n")
        if not txt:
            if not messagebox.askyesno("Empty text", "Write an empty payload (will become all zeros)?"):
                return
